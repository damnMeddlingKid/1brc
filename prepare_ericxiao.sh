...
if [ ! -f target/CalculateAverage_ericxiao_image ]; then
    ...
    NATIVE_IMAGE_OPTS="-g -H:+SourceLevelDebug -H:-DeleteLocalSymbols -O3 -H:TuneInlinerExploration=1 -march=native"
    ...
    NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS --enable-preview --initialize-at-build-time=dev.morling.onebrc.CalculateAverage_ericxiao_image\$ProcessFileMap"
    ...
    # NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS -H:MethodFilter=CalculateAverage_ericxiao_image.* -H:Dump=:2 -H:PrintGraph=Network"
